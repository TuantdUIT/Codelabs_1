import { ionLabel } from './ions';
import type { ActiveIon } from './ions';
import type { GameEngine } from './engine';
import * as G from './grid';

export function drawFrame(ctx: CanvasRenderingContext2D, engine: GameEngine): void {
...
function drawDeathLine(ctx: CanvasRenderingContext2D): void {
  ctx.save();
  ctx.setLineDash([8, 8]);
...
function drawBubble(ctx: CanvasRenderingContext2D, x: number, y: number, ion: ActiveIon, radius: number): void {
  ctx.save();
  ctx.beginPath();
...
function drawAim(ctx: CanvasRenderingContext2D, engine: GameEngine): void {
...
function drawCannon(ctx: CanvasRenderingContext2D, engine: GameEngine): void {
