...
export interface Ion {
  ...
  polyatomic: boolean;
  color: string;
  /** Tên axit tương ứng, chỉ có ở anion; null khi cặp đó không tạo axit. */
  acidName?: string | null;
}

/** Ion đã được gán màu và phía (+/−) cho một ván chơi cụ thể. */
export interface ActiveIon extends Ion {
  type: IonType;
}
...
export function toSubscript(str: string | number): string {
  return String(str).replace(/\d/g, (d) => SUB_DIGITS[d]);
}

export function toSuperscript(str: string | number): string {
  return String(str)
    .split('')
    .map((c) => SUP_CHARS[c] ?? c)
    .join('');
...
export function ionLabel(ion: Ion, type: IonType): string {
...
function buildIonSet(pool: Ion[], ids: string[], colors: string[], type: IonType): ActiveIon[] {
  return pool
    .filter((ion) => ids.includes(ion.id))
    .map((ion, i) => ({ ...ion, color: colors[i % colors.length], type }));
}

export const buildCations = (ids: string[]): ActiveIon[] => buildIonSet(CATIONS, ids, CATION_COLORS, 'cation');
export const buildAnions = (ids: string[]): ActiveIon[] => buildIonSet(ANIONS, ids, ANION_COLORS, 'anion');

export function pickSubset(pool: Ion[], count: number, rng: () => number = Math.random): Ion[] {
  const byCharge: Record<number, Ion[]> = {};
  for (const ion of pool) {
    (byCharge[ion.charge] ??= []).push(ion);
  }
  const picked = new Map<string, Ion>();
  // guarantee at least one ion of each available charge tier first
  for (const charge of Object.keys(byCharge)) {
    const options = byCharge[Number(charge)];
