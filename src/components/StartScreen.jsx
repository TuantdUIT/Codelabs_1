...
import IonSelect from './IonSelect.jsx';
...
import { possibleCompounds } from '../game/chemistry.js';
import { DIFFICULTIES } from '../game/organic-engine.js';
...
export default function StartScreen({ setup, music, onStart }) {
  const [mode, setMode] = useState(setup?.mode ?? 'inorganic');
  const [difficultyId, setDifficultyId] = useState(setup?.difficultyId ?? 'easy');
  const [cationIds, setCationIds] = useState(setup?.cationIds ?? DEFAULT_CATION_IDS);
  const [anionIds, setAnionIds] = useState(setup?.anionIds ?? DEFAULT_ANION_IDS);
  ...
  const colorOfCation = (id) => cations.find((ion) => ion.id === id)?.color ?? 'transparent';
  const colorOfAnion = (id) => anions.find((ion) => ion.id === id)?.color ?? 'transparent';
